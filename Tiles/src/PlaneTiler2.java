...
public class PlaneTiler2 {
    public ArrayList<Plane2> _solutions;
    ...
    private ArrayList<Tile2> candidates(Plane2 plane, int x, int y) {
        ArrayList<Tile2> candidates = new ArrayList<>();

        for (Tile2 tile : _tiles) {
            if (!tile._isPlaced &&
    ...
    public void tiler() {
        tiler(new Plane2(_width, _height), 1, 1, System.currentTimeMillis());
    }

    private void tiler(Plane2 plane, int x, int y, long start) {
        // base case 1 - plane is full
        if (plane._plane[_height-2][_width-2]._id != -1) {
            for (Plane2 solution : _solutions) {
                if (plane.isomorphic(solution)) {
                    return;
                }
            }

            _solutions.add(new Plane2(plane));
            return;
        }

        ArrayList<Tile2> candidates = candidates(plane, x, y);
        ...
        for (Tile2 candidate : candidates) {
        ...
        plane.planarSet(nextX, nextY, new Tile2());
